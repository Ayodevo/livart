            ...
            <div class="col-lg-12 mb-3 mb-lg-0">
                <div class="card mb-3 mb-lg-5">
                    ...
                    <div class="row justify-content-center justify-content-md-end mb-3">
                        ...
                        @else
                            <div class="col-md-9 col-lg-8">
                                <a class="btn btn-primary btn-block btn-lg" data-bs-toggle="modal" data-bs-target="#paymentModel">
                                    <i class="tio-money"></i> {{ translate('payment') }}
                                </a>
                            </div>
                        @endif
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel">Choose method payment</h5>
                                        </div>
                                        <div class="modal-body">
                                            @foreach($settings as $setting)
                                                <div class="card mb-3" onclick="selectSetting('{{ $setting->key }}')">
                                                    <div class="card-body">
                                                        <h5 class="card-title">key: {{ $setting->key }}</h5>
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="radio" name="setting_key" id="{{ $setting->key }}" value="{{ $setting->key }}">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-primary" id="stripeBtn"   style="display: none;" onclick="paymentWith('stripe')">Payment With Stripe</button>
                                            <button type="button" class="btn btn-primary" id="paypalBtn"   style="display: none;" onclick="paymentWith('paypal')">Payment With Paypal</button>
                                            <button type="button" class="btn btn-primary" id="razorpayBtn" style="display: none;" onclick="paymentWith('razorpay')">Payment With RazorPay</button>
                                        </div>

                                        <script>
                                            function selectSetting(key) {
                                                document.getElementById(key).checked = true;
                                                // Afficher le bouton approprié en fonction de la méthode de paiement sélectionnée
                                                document.getElementById('stripeBtn').style.display = key === 'stripe' ? 'inline-block' : 'none';
                                                document.getElementById('paypalBtn').style.display = key === 'paypal' ? 'inline-block' : 'none';
                                                document.getElementById('razorpayBtn').style.display = key === 'razor_pay' ? 'inline-block' : 'none';
                                            }

                                            function paymentWith(method) {
                                                // Vous pouvez mettre en œuvre le comportement souhaité ici
                                                console.log('Payment with ' + method);
                                                // Par exemple, vous pouvez rediriger l'utilisateur vers la page de paiement appropriée
                                            }
                                        </script>
